...
def modelC(x_train, y_train):
    ...
    model.compile(loss=tf.losses.categorical_crossentropy, #cross_entropt loss function, adadelta optimizer
             ...
             metrics=['accuracy'])
    model.summary()
    history = model.fit(x_train, to_categorical(y_train), epochs = 80, shuffle=True, validation_split=0.33)
    return history
...
#history.model.evaluate(x_test, to_categorical(y_test)) #uncomment for non bootstrap result
...
number_of_bootstraps = 100 #industry recommended is 1k ~ 10k 
...
print('fNN validation train accuracy: Min =',min(train_acc),'Max =',max(train_acc))
fnn_train_mean = np.mean(train_acc)
fnn_train_stderr = np.std(train_acc)/np.sqrt(number_of_bootstraps)
fnn_train_CI = (fnn_train_mean - 1.96*fnn_train_stderr, fnn_train_mean + 1.96*fnn_train_stderr) #95% confidence
print('95% confidence interval of fNN validation train accuracy:', fnn_train_CI)

print('fNN test accuracy: Min =',min(test_acc),'Max =',max(test_acc))
fnn_test_mean = np.mean(test_acc)
fnn_test_stderr = np.std(test_acc)/np.sqrt(number_of_bootstraps)
fnn_test_CI = (fnn_test_mean - 1.96*fnn_test_stderr, fnn_test_mean + 1.96*fnn_test_stderr) #95% confidence
print('95% confidence interval of fNN test accuracy:', fnn_test_CI)
###BOOTSTRAP
